 ...
 */
sisob.euParliament.visualizations.keywordsEvolution = function (pJsonData) {
    ...
    d3.selectAll("#chart1").append("select")
        ...
        .selectAll("option")
            ...
            .text( function(d){
                var categoryFormatted = d.replace(/_/g, " ");
    ...
    function getData(dataLC){
        ...
        var categorySelected = sisob.util.getSelect_value('idSelectOntologyType');
        ...
        try{
            ...
            $.each(listKeywords, function (indexKeyword, iKeyword) {
                ...
                $.each(iKeyword.values, function (indexKeyword2, currentValue) {
                    ...
                    if(lastDate != null){
                        lastDate = sisob.util.addDays(lastDate , 1);
                        if( !(sisob.util.isDateEquals(lastDate, currentValue.x)) ){
                            var newData = { x:lastDate, y:0};
                            valuesTemp.push(newData); // console.log(iKeyword.key + " - "+ newData.x);

                            var previousCurrentDate = sisob.util.addDays(currentValue.x , -1);
                            if( !(sisob.util.isDateEquals(previousCurrentDate, lastDate)) ){
            ...
            for(var iK in listKeywords){
                ...
                if (Object.keys(iKeyword.values).length === 1){
                    ...
                    var previousDay = sisob.util.addDays(iDate, -1);
                    var nextDay = sisob.util.addDays(iDate, 1);
